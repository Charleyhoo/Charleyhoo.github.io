# **C++ Vscode Config**
- **Creat a folder called .vscode and add three files: `setting.json`, `task.json` and  `Launch.json`**
- **Download code runner in the extension**
- **Set up the `format Google` and choose the theme `Dark+` in the settings**
- **Search the shortcut key and change the code runner ---> run program in `command+R`**
## Folder(.vscode)
**Claim: Have some references from others.**
### Code Runner(setting.json)
```json
{
    // --------------------------------------------------------------------------------------
    // Code Runner
